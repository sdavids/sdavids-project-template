...
import (
	...
	"sdavids.de/sdavids-project-template/internal/logger"
	"sdavids.de/sdavids-project-template/internal/server"
...
func run(ctx context.Context, opts runOptions) (int, error) {
	...
	config, err := opts.newConfig(opts.getenv, opts.unsetenv)
	if err != nil {
		var e *sysexits.SysExitError
		if errors.As(err, &e) {
	...
	code, err := opts.runServer(ctx, opts.logger, config)
	if err != nil {
